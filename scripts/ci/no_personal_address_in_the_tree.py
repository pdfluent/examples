...
#
# CI tooling for this examples repository, covered by the repository's MIT
# licence -- see LICENSE. Copied from the PDFluent engine repository, where the
# original is maintained; changes there do not arrive here on their own.
...
# In a small repository 500 is a floor no honest scan can clear, and a gate that
# can only fail is not a gate. The value moves with the repository; what it has
# to catch is the scan that read nothing at all, and any floor above zero does
# that. It is not a boundary against tampering -- whoever can edit the workflow
# can edit this file -- so it is set where the repository actually sits.
MINIMUM_BESTANDEN = int(os.environ.get("MINIMUM_BESTANDEN", "500"))

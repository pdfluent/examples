...
def einde_tot_eind(fouten: list[str]) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for email, moet_falen in ((ALIAS, False), ("t@t", True)):
            ...
            # The name as well as the address. Configuring half an identity
            # leaves the other half to the machine: git derives a name from
            # /etc/passwd, so this passed on a laptop and died on a runner
            # whose user has no gecos, with "Author identity unknown" from a
            # guard that is supposed to be judging the address.
            _git(wd, "config", "user.name", "fixture")
            r = subprocess.run(
